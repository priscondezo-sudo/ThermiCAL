...
if uploaded_file is not None and A is not None:
    ...
    # Crear máscara de nodata ANTES de calibrar
    ...
    # ── Detección automática: si los valores válidos superan 100°C
    #    se asume que están en décimas de grado → dividir entre 10
    max_valid = np.nanmax(image[valid]) if valid.any() else 0
    if max_valid > 100:
        image_to_calibrate = image.copy()
        image_to_calibrate[valid] = image[valid] / 10.0
        st.warning(f"⚠️ Se detectaron valores mayores a 100°C (máx: {max_valid:.1f}). "
                   f"La imagen fue dividida entre 10 antes de aplicar la calibración.")
    else:
        image_to_calibrate = image.copy()

    # Aplicar calibración SOLO a píxeles válidos
    calibrated = image_to_calibrate.copy()
    calibrated[valid] = A * image_to_calibrate[valid] + B

    # Restaurar nodata en la imagen calibrada
    if nodata is not None:
    ...
    preview_orig = np.where(valid, image_to_calibrate, np.nan)
    ...
    # Guardar conservando georreferenciación y nodata
