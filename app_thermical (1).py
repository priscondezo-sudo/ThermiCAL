...
if uploaded_file is not None and A is not None:
    ...
    if nodata is not None:
        ...
        # Si no hay nodata definido, asumir que 0 es nodata (borde negro)
        mask_nodata = (image == 0)

    # Aplicar calibración SOLO a píxeles válidos
    calibrated = image.copy()
    valid = ~mask_nodata
    calibrated[valid] = A * image[valid] + B
    ...
    # Vista previa original (solo píxeles válidos)
    st.markdown("#### 🗾 Imagen Original")
    preview_orig = np.where(valid, image, np.nan)
    ...
    # Mostrar estadísticas calibradas
